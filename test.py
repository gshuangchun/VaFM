...
sol = {"cvrp": {50: 10.287, 100: 15.543},
       "vrpb": {50: 9.688, 100: 14.386},
       "vrpbl": {50: 9.688, 100: 14.373},
       "vrpbltw": {50: 18.361, 100: 29.026},
       "vrpbtw": {50: 18.167, 100: 29},
       "vrpl": {50: 10.328, 100: 15.637},
       "vrpltw": {50: 15.951, 100: 25.678},
       "vrptw": {50: 16.032, 100: 25.433},
       "ovrp": {50: 6.494, 100: 9.73},
       "ovrpb": {50: 6.897, 100: 10.304},
       "ovrpbl": {50: 6.904, 100: 10.31},
       "ovrpbltw": {50: 11.597, 100: 19.005},
       "ovrpbtw": {50: 11.59, 100: 19.167},
       "ovrpl": {50: 6.51, 100: 9.709},
       "ovrpltw": {50: 10.455, 100: 16.962},
       "ovrptw": {50: 10.485, 100: 16.9},
       }
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--checkpoint", type=str, default="", help="Path to the model checkpoint"
    )
    parser.add_argument(
        "--problem",
        type=str,
    ...
    parser.add_argument(
        "--datasets",
        help="Filename of the dataset(s) to evaluate. Defaults to all under data_ori/{problem}/ dir",
        default=None,
    )
    ...
    opts.remove_mixed_backhaul = True
    # opts.checkpoint = './logs/1018-routefinder-vis-fusion-loc-depot-CBLTW-value-xL-orange-01cls-nodeAtt-2dec-avg/2024-10-19_14-08-10/checkpoints/epoch_299.ckpt'
    # opts.checkpoint = './logs/1018-routefinder-vis-fusion-loc-depot-CBLTW-value-xL-orange-01cls-nodeAtt-2dec-avg-50/2024-10-22_09-37-43/checkpoints/epoch_299.ckpt'
    # opts.batch_size = 150
    ...
    if opts.datasets is not None:
        ...
        # list recursively all npz files in data_ori/
        data_paths = []
        for root, _, files in os.walk("data"):
            for file in files:
                ...
                if "test" not in root:
                    continue
                if file.endswith("{}.npz".format(str(opts.size))):
                    if opts.remove_mixed_backhaul and "mb" in root:
                        continue
                    # if name in 50 or 100, append
                    if str(opts.size) in file:
                        data_paths.append(os.path.join(root, file))
        assert len(data_paths) > 0, "No datasets found. Check the data_ori directory."
        data_paths = sorted(sorted(data_paths))
    ...
    for dataset in tqdm(data_paths):
        # if not dataset.endswith("200.npz"):
        #     continue
        ...
        # out["gap_to_bks"] = out["max_aug_reward"] - sol

        inference_time = time.time() - start

        dataset_name = dataset.split("/")[-3].split(".")[0]
        # out["gap_to_bks"] = out["max_aug_reward"]
        out["gap_to_bks"] = (-1 * out["max_aug_reward"] - sol[dataset_name][opts.size]) / sol[dataset_name][
            opts.size] * 100
    ...
    if opts.save_results:
        ...
        with open(savedir + checkpoint_name + ".json", 'w') as json_file:
